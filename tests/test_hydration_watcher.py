...
def test_hydration_watcher_no_candidates():
    """Verify run_once returns None when no issues match filter."""
    mock_factory = MagicMock()
    mock_app = MagicMock()
    mock_task_service = MagicMock()
    mock_factory.get_task_service.return_value = mock_task_service
